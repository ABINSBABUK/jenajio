...
def run_add_session_data(self):
    ...
    #add session tag
    self.hs_api_call.add_session_tags(
        self.session_id,status=self.status,message_pass_count=self.pass_count,
        message_fail_count=self.fail_count,#session_url=str(self.session_url),
        os_type=str(self.os)#,machine=str(self.host_name),net_speed=str(self.net_speed)
        )
    #,browser=self.browser,env=self.env, removed from above
...
def get_general_session_data(self):
    ...
    # app info
    session_data['data'].append({"key": kpi_names.BUNDLE_ID, "value": "chrome"})
    ...
    # email_content="RUN OBSERVATIONS\n" + self.test_name + " \n" + email_message_formation(self,session_data)
    # email_subject=email_subject_creation(self,session_data,email_message_formation(self,session_data))
    # if len(email_subject)>0:
    #     SendAlert.email_alert(self,email_subject,email_content)
    # print(email_content)

    # if self.genre_id:
    #     session_data['data'].append({"key": kpi_names.GENRE_ID, "value": "id:"+self.genre_id})
    ...
    #        session_data['data'].append({"key": kpi_names.APP_SIZE_PRE_LAUNCH, "value": self.app_size_info_pre_launch['app_size']})
    #        session_data['data'].append({"key": kpi_names.USER_DATA_PRE_LAUNCH, "value": self.app_size_info_pre_launch['user_data']})
    #        # session_data['data'].append({"key": kpi_names.CACHE_PRE_LAUNCH, "value": self.app_size_info_pre_launch['cache']})
    #        session_data['data'].append({"key": kpi_names.TOTAL_PRE_LAUNCH, "value": self.app_size_info_pre_launch['total']})
    #except:
    #    pass    
    #try:
    #    if self.app_size_info_post_launch:
    #        # session_data['data'].append({"key": kpi_names.APP_SIZE_POST_LAUNCH, "value": self.app_size_info_post_launch['app_size']})
    #        session_data['data'].append({"key": kpi_names.USER_DATA_POST_LAUNCH, "value": self.app_size_info_post_launch['user_data']})
    #        # session_data['data'].append({"key": kpi_names.CACHE_POST_LAUNCH, "value": self.app_size_info_post_launch['cache']})
    #        session_data['data'].append({"key": kpi_names.TOTAL_POST_LAUNCH, "value": self.app_size_info_post_launch['total']})
    #except:
    #    pass
    ...
    #    session_data['data'].append({"key": kpi_names.CODE_VERSION, "value": self.code_version})
