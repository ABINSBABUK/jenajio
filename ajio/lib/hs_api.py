...
class hsApi:
    ...
    #API for getting all devices and its details present in  an org
    def __init__(self, UDID=None, access_token=None):
        self.UDID = UDID
...
#		return devices
    ...
    def get_tls_exceptions(self, session_id):
        request_url = self.url_root + 'v0/sessions/' + session_id + '/tlsexceptions'
        response = requests.get(request_url, headers=self.headers)
        return self.parse_response(response)

    def get_devices(self):
        request_url = self.url_root + 'v0/devices'
        response = requests.get(request_url, headers=self.headers)
        return self.parse_response(response)
    ######################### Mangae the device state on HS platform
    
    def db_sync(self, test_name):
        api_endpoint = "https://api-dev.headspin.io/v0/perftests"
        r = requests.get(url=api_endpoint,  headers={'Authorization': 'Bearer {}'.format(self.access_token)} )
        r = r.json()
        perf_tests = r['perf_tests']
        for perf_test in perf_tests:
            name = perf_test['name']
            if name == test_name:
                perf_id = perf_test['perf_test_id']
                print(perf_id)
                api_endpoint = "https://api-dev.headspin.io/v0/perftests/{}/dbsync".format(perf_id)
                r = requests.post(url=api_endpoint,  headers={'Authorization': 'Bearer {}'.format(self.access_token)} )
                print(r.text)
